...
update_sh() {
    ol_version=$(curl -L -s https://raw.githubusercontent.com/centpourcent/V2Ray_ws-tls_bash_onekey/${github_branch}/install.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        ...
        [yY][eE][sS] | [yY])
            wget -N --no-check-certificate https://raw.githubusercontent.com/centpourcent/V2Ray_ws-tls_bash_onekey/${github_branch}/install.sh
